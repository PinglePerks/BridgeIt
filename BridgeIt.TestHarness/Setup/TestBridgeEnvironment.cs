...
public class TestBridgeEnvironment
{
    ...
    public TestBridgeEnvironment WithRulesFromDirectory(string directoryPath)
    {
        var loader = Provider.GetRequiredService<YamlRuleLoader>();
        var rules = loader.LoadRulesFromDirectory(directoryPath);
        
        // Re-register or Instantiate Engine with these specific rules
        Engine = new BiddingEngine(rules);
        RebuildTable();
        return this;
    }
