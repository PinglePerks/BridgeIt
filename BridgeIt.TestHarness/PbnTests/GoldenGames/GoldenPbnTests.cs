...
public class GoldenPbnTests
{
    ...
    public static IEnumerable<TestCaseData> GetOtherHands()
    {
        // 1. Get the path to the folder itself
        var folderPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "TestFiles", "OtherGames");

        // Safety check: Make sure the folder exists so the test runner doesn't mysteriously crash
        if (!Directory.Exists(folderPath))
        {
            throw new DirectoryNotFoundException($"Could not find OtherHands folder at: {folderPath}");
        }
